...
const ProductListHome = ({type, bg = "", className = "" }) => {
  const dispatch = useDispatch()
  useEffect(() => {

        dispatch(getProduct(type)).then((dataThen)=>{
setData(dataThen.payload.data.data)
        })
  }, []);
  ...
  return (
    <div className={`${className}`}>
      <div
        className={`container ${
          bg === "bg1" ? 'bg-[url("../images/bg-hot.jpg")] h-[460px]' : ""
        }
        ${bg === "bg2" ? 'bg-[url("../images/bg-laptop-1.png")] h-[460px]' : ""}
        ${bg === "bg3" ? 'bg-[url("../images/bg-laptop.png")] h-[460px]' : ""}
