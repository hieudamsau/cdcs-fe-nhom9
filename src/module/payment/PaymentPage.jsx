...
const PaymentPage = () => {
  ...
  //     client_id: 'AQyiOqbDckrBsb3Gj5IMJHUtsS2McZuqZiMQfmw7ewodrTcKj0h8IAcxIt61TTt6hPCxW_Zj_6-E3Qjw',
  //     client_secret: 'EDQmuf-hOCPKtWrN31OMcWddw_RLRnFHnWJ1h0vA63yl2Q-oxB9elNjrpCVbnkCVWzvQUoGEicIsAl-G'
  //   });
  // }, []);
  
  const price = (cart?.reduce((count, item) => count + item.quantity * item.product.price,0))/ 23000
  const total = parseInt(price)
  
  const createOrder = (data, actions) => {
    // Logic để tạo đơn hàng và trả về ID đơn hàng cho PayPal
  ...
  const onApprove = (data, actions) => {
    // Xử lý khi người dùng đã chấp nhận thanh toán
    return actions.order.capture().then(function (details) {
      ...
      const dataAdress = {
        ...
        try{
          dispatch(resetCart());
          const response = orderApi.createOrder(dataAdress);
        }catch (error) {
          console.log(error.message);
        }
        navigate("/account/orders");
              ...
              <div className="flex flex-col bg-white rounded-lg pb-5 mt-10">
                ...
                {paymentMethod === "paypal" ?
                  <PayPalScriptProvider  options={{ 
                    'client-id': 'AUVH-i5IA6r_nwWPSnWBERknak0TqaESoSbRoMetQOkKHRsA1mZ46MiHc_KBU-6V1EPP4q8J-pzekN4Q' ,
                    }}>
                    <PayPalButtons
