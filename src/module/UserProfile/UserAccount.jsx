...
import { getUser, getUserByToken, refresh, updateInfoUser } from "../../redux/auth/userSlice";
...
const {userByToken}=useSelector(state=>state.user)
  useEffect(() => {
    dispatch(getUserByToken());
  }, []);
  console.log("token",userByToken);

  useEffect(() => {
    reset({
      fullname: userByToken?.full_name,
      image: userByToken?.avatar,
      email: userByToken?.email,
      phone: userByToken?.phone,
      gender: userByToken?.gender,
    });
  ...
  const handleUpdate = (values) => {
    ...
    cloneValues.phone = values.phone;
    try {
        ...
        {status === action_status.SUCCEEDED && (
          <form className="pb-16" onSubmit={handleSubmit(handleUpdate)}>
            ...
            <Field>
              ...
              <Input name="phone" type="number" control={control}></Input>
              {errors.phone && (
                <p className="text-red-500 text-base font-medium">
